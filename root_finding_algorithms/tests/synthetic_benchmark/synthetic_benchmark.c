 ...
 */
void run_a_test(cqk_problem *restrict p, char *restrict method,
    ...
    fprintf(out, "%d\t%e\t", status, duration);
}
...
int main(int argc, char *argv[]) {
    ...
    char *methods[4] = {"fixed_point", "secant", "regula_falsi", "bissection"};
    ...
    for (unsigned repetitions = 0; repetitions < ntests; ++repetitions) {
        ...
        for (unsigned met = 0; met < n_methods; ++met)
            run_a_test(&p, methods[met], out, x);
        fprintf(out, "\n");
    }
    free_cqk_problem(&p);
    return 0;
